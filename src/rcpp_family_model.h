...
public:
  ...
  virtual double inverse_hazardRate(double x) = 0;
	...
	virtual double hazardRate_param_derivative(double x) = 0;

	virtual double cumulative_hazardRate_param_derivative(double x) = 0;

  virtual double hazardRate_derivative_param_derivative(double x) = 0;//LD

  virtual double hazardRate_2derivative(double x) = 0;//LD

  virtual double hazardRate_param_2derivative(double x) = 0;//LD

  virtual double cumulative_hazardRate_param_2derivative(double x) = 0;//LD
};
...
public:
  ...
  double hazardRate_param_derivative(double x) {
  	return (x==0 ? 0 : alpha*(1+beta*log(x))*pow(x,beta-1));
  }

  double cumulative_hazardRate_param_derivative(double x) {
  	return (x==0 ? 0 : alpha*log(x)*pow(x,beta));
  }

  double hazardRate_derivative_param_derivative(double x) {//LD
  ...
  double hazardRate_param_2derivative(double x) {//LD
    return (x==0 ? 0 : alpha*(2+beta*log(x))*log(x)*pow(x,beta-1));//LD
  }//LD

  double cumulative_hazardRate_param_2derivative(double x) {//LD
