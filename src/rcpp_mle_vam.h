...
public:
    ...
    NumericVector contrast(NumericVector param, bool alpha_fixed=false) {
        ...
        if(!alpha_fixed)
            ...
            {res[0]=log(alpha)*S3+S2-alpha*S1;}
    ...
    void gradient_for_current_system() {
        ...
        S1 += model->S1;S2 += model->S2; S3 += model->S3;
        for(int i=0;i<(model->nb_paramsMaintenance+model->nb_paramsFamily-1);i++) {
    ...
    NumericVector gradient(NumericVector param, bool alpha_fixed=false) {
        ...
        if(!alpha_fixed) {
            res[0] = 0;
            for(int i=0;i<(model->nb_paramsMaintenance+model->nb_paramsFamily-1);i++) {
                res[i+1] = -dS1[i]/S1 * S3 + dS2[i];
            }
        } else {

            res[0] = S3/alpha-S1;
            for(int i=0;i<(model->nb_paramsMaintenance+model->nb_paramsFamily-1);i++) {
                res[i+1] = dS2[i]-alpha*dS1[i];
            }
        }
    ...
    NumericMatrix hessian(NumericVector param, bool alpha_fixed=false) {
        ...
        if(!alpha_fixed) {
            ...
            for(int i=0;i<(model->nb_paramsMaintenance+model->nb_paramsFamily-1);i++) {
                res(0,i+1) = 0;
                res(i+1,0) = 0;
                res(i+1,i+1) = pow(dS1[i],2)/pow(S1,2) * S3-d2S1[i*(i+1)/2+i]/S1 * S3 + d2S2[i*(i+1)/2+i];
                for(j=0;j<i;j++) {
                    //i and j(<=i) respectively correspond to the line and column indices of (inferior diagonal part of) the hessian matrice
                    res(i+1,j+1) = dS1[i]*dS1[j]/pow(S1,2) * S3-d2S1[i*(i+1)/2+j]/S1 * S3 + d2S2[i*(i+1)/2+j];
                    res(j+1,i+1) = res(i+1,j+1);
                }
            }
        } else {
            
            res(0,0) = -S3/pow(alpha,2);
            for(int i=0;i<(model->nb_paramsMaintenance+model->nb_paramsFamily-1);i++) {
                res(0,i+1) = -dS1[i];
                res(i+1,0) = -dS1[i];
